function f = Likelihood_NoPreempt(Theta,output_type)
% This new formulation of "no-preemption" counterfactual makes entrants behave *as if* incumbents never innovated
% This version: 11/14/2013
...
%tic;
    ...
    % Note: No-preemption CF should alter "terminal values" in the main m-file, too.
    ...
    for no = 0:11               % Circle around Today's # of Old firms
        for nb = 0:11           % Circle around Today's # of Both firms
            for nn = 0:14       % Circle around Today's # of New firms
                ...
                % Entrants perceive states differently, ignoring incumbents' innovations
                no2 = (no + nb)*(no + nb <= 11) + 11*(no + nb > 11);    % No* = No + Nb
                nb2 = 0;                                                % Nb* = 0
...
%    , no, nb, nn, Npe, Npe_prime);    
                ...
                while (gap > .01 && iterNE < 10)   
                    ...
                    if nn > 0
                        % Entrants perceive no2 & nb2 (instead of no & nb) in no-preemption CF
                        z4 = fun4(z6, z7, z8, z9old, z10old,...
                            no2, nb2, nn, Npe, Npe_prime, Vprime);
...
if nn > 0
% Entrants perceive no2 & nb2 (instead of no & nb) in no-preemption CF    
V(t,3,no+1,nb+1,nn+1) = Pi(t,3,no2+1,nb2+1,nn+1) + 0.57722 +...
    fun13(z4, beta, phi);
    %log(exp(0) + exp(phi + beta * EV(t+1,4,no2+1,nb2+1,nn+1)));
