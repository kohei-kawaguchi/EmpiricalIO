...
compute_payoff_across_markets <-
  function(Y, X, Z, EP, NU, beta, alpha, delta, rho) {
    payoff <-
      foreach (m = 1:length(Y), .packages = c("EmpiricalIO", "foreach", "magrittr")) %dopar% {
        y_m <- Y[[m]]
        # extract
        X_m <- X[m, , drop = FALSE]
        Z_m <- Z[[m]]
...
# unobserved entrant attributes
NU_mc <-
  foreach (r = 1:R, .packages = c("EmpiricalIO", "foreach", "magrittr")) %dopar% {
  NU <-
    foreach (m = 1:M) %do% {
      NU_m <- matrix(
        rnorm(E[m]),
...
compute_monte_carlo_sequential_entry <-
  function(X, Z, EP_mc, NU_mc,
    ...
    Y_mc <-
      foreach (r = 1:length(EP_mc), .packages = c("EmpiricalIO", "foreach", "magrittr")) %dopar% {
        # extract
        EP_r <- EP_mc[[r]]
        NU_r <- NU_mc[[r]]
        Y_r <-
...
compute_objective_sequential_entry <-
  function(Y, X, Z, EP_mc, NU_mc, theta) {
    ...
    objective <-
      foreach (r = 1:length(EP_mc), .combine = "rbind", .packages = c("EmpiricalIO", "foreach", "magrittr")) %dopar% {
        Y_mc_r <- Y_mc[[r]]
