function f = Likelihood_PatentExAnte(Theta,output_type)
% This file is to solve the "ex-ante broad patent" counterfactual
    ...
    for no = 0:11               % Circle around Today's # of Old firms
        for nb = 0:11           % Circle around Today's # of Both firms
            for nn = 0:14       % Circle around Today's # of New firms
                ...
                % Flag = 1 if someone is currently making new HDDs
                if nb + nn > 0
                    flag = 1;   
                else
                    flag = 0;
                ...
                % When flag = 1, no new entry allowed
                if flag == 1
                    Npe = 0;
                    Npe_prime = 0;
                end
                ...
                % Old-only's choice set depends on "flag" 
                % (i.e., can't innovate if someone is already making new HDDs)
                if no > 0
                    if flag == 0
...
% Old-only's choice set (and expected value) depends on "flag"
if no > 0
    if flag == 0
