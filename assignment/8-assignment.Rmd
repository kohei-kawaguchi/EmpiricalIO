...
```{r}
PI <- compute_PI_game(alpha, beta, eta, N, A, S)
head(PI[[N]])
tail(PI[[N]])
```
...
```{r}
G <- compute_G_game(kappa, gamma, L, K, A, S)
G[1, ]
G[m_a, ]
A[m_a,]
G[m_a, 1] - (1 - gamma)^N
...
p_marginal <- matrix(rep(1/(K + 1), (K + 1) * m_s), nrow = m_s) 
colnames(p_marginal) <- paste("a", 0:K, sep = "")
rownames(p_marginal) <- paste("l", 1:m_s, sep = "")
...
compute_p_joint <-
  function(p_marginal, A, S) {
    ...
    p_joint <- 
      foreach (l = 1:m_s) %do% {
        p_marginal_l <- p_marginal[l, ]
        p_joint_l <- matrix(p_marginal_l[A + 1], nrow = dim(A)[1])
        p_joint_l <- t(matrix(apply(p_joint_l, 1, prod)))
      }
    return(p_joint)
  }
