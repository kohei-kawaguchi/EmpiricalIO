...
2. Write a function that returns the log output given $l_{jt}$, $k_{jt}$, $\omega_{jt}$, and $l_{jt}$ under the given parameter values according to the above production function and name it `log_production`.
```{r}
...
3. Compute a function that returns the value of $g_{JT}(\alpha, \beta_0, \beta_k)$ given parameter values, data, and `df_T_1st`, and name it `moment_OP_2nd`. Show the values of the first five rows of the moment evaluated at the true parameters.
```{r}
moment_OP_2nd <- 
...
head(moment_OP_2nd(alpha, beta_0, beta_k, df_T, df_T_1st))
```
