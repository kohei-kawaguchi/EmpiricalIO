...
```{r}
load(file = "data/A4_result_NLLS.RData")
result_NLLS
...
```{r, echo = TRUE}
load(file = "data/A4_delta_new.RData")
summary(delta_new$delta - delta$delta)
```
...
objective <-
  compute_gmm_objective_a4(theta_nonlinear, delta, df_share_smooth, Psi, 
                   X, M, V_mcmc, kappa, lambda) 
save(objective, file = "data/A4_objective.RData")
```
```{r, echo = TRUE}
load(file = "data/A4_objective.RData")
objective
```
  ...
  objective_i <- 
    foreach (theta_nonlinear_ij = theta_nonlinear_i_list,
             ...
             .combine = "rbind") %dopar% {
               ...
               objective_ij <-
                 compute_gmm_objective_a4(theta_nonlinear_j, delta, df_share_smooth, Psi, 
                                  X, M, V, kappa, lambda) 
               return(objective_ij)
             }
  df_graph <- 
...
```
```{r}
load(file = "data/A4_graph_true.RData")
graph_true
```
...
```{r}
load(file = "data/A4_result.RData")
result
