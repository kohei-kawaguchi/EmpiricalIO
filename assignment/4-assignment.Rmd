...
In the previous assingment, we computed predicted share by simulating choice and taking their average. Instead, we compute the predicted share by:
$$
...
4. Write a function `compute_choice_smooth(X, M, V, beta, sigma, mu, omega)` in which the choice of each consumer is not:
$$
q_{ijt} = 1\{u_{ijt} = \max_{k \in \mathcal{J}_t \cup \{0\}} u_{ikt}\},
$$
but
...
$$
Then, write a function `compute_share_smooth(X, M, V, beta, sigma, mu, omega)` that calls `compute_choice_smooth` and then returns the share based on above $\tilde{q}_{ijt}$. If we use these functions with the Monte Carlo shocks, it gives us the predicted share of the products. 
...
8. Write a function `solve_delta(df_share, X, M, V, delta, sigma, mu, omega)` that finds $\delta_{jt}$ that equates the actua share and the predicted share based on `compute_share_smooth_delta` by the fixed-point algorithm with an operator:
$$
...
Start the algorithm with the true $\delta_{jt}$ and check if the algorithm returns (almost) the same $\delta_{jt}$ when the actual and predicted smooth share are equated (we match non-smooth share `df_share` in the analysis but here only for debugging purpose we match smooth share `df_share_smooth`.).
...
9. Check how long it takes to compute the limit $\delta$ under the Monte Carlo shocks starting from the true $\delta$ to match with `df_share`. This is approximately the time to evaluate the objective function.
```{r, echo = TRUE}
system.time(
delta_new <-
  solve_delta(df_share, X, M, V_mcmc, delta, sigma, mu, omega, kappa, lambda)
)
summary(delta_new$delta - delta$delta)
```


10. We use the marginal cost $c_{jt}$ as the excluded instrumental variable for $p_{jt}$. Let $\Psi$ be the weighing matrix for the GMM estimator. For now, let it be the identity matrix. Write a function `compute_theta_linear(df_share, delta, Psi)` that returns the optimal linear parameters associated with the data and $\delta$. Notice that we only obtain $\beta_0$ in this way because $\alpha_0$ is directly computed from the non-linear parameters by $-\exp(\mu + \omega^2/2)$.
\begin{equation}
\theta_1 = (X'W \Phi^{-1} W'X)^{-1} X' W \Phi^{-1} W' \delta,
\end{equation} 
where
\begin{equation}
X = 
...
\end{pmatrix}
\end{equation}.
Notice that $X_1$ and $W$ does not include rows for the outwide option.
...
11. Write a function `GMM_objective_A4(theta_nonlinear, delta, df_share, Psi, X, M, V_mcmc, kappa, lambda)` that returns the value of the GMM objective function as a function of non-linear parameters `mu`, `omega`, and `sigma`:
$$
...
theta_nonlinear <- c(mu, omega, 3*sigma)
