...
5. Write a function `initialize_p_marginal(A, S)` that defines an initial marginal condition choice probability. In the output `p_marginal`, `p` is the probability for firm `i` to take action `a` conditional on the state profile being `l`. Next, write a function `compute_p_joint(p_marginal, A, S)` that computes a corresponding joint conditional choice probability from a marginal conditional choice probability. In the output `p_joint`, `p` is the joint probability that firms take action profile `k` condition on the state profile being `l`.
...
df <- simulate_dynamic_decision_across_markets(p_joint, l, G, N, T, M, S, A, seed)
...
$$
where $\hat{p}$ is the non-parametric estimate of the joint conditional choice probability and $p^{(\theta_1, \theta_2)}$ is the equilibrium join conditional choice probability under parameters $\theta_1$ and $\theta_2$. $a_k$ is $k$-th action profile and $s_l$ is $l$-th state profile.
...
p_joint_est <- compute_p_joint(p_marginal_est, A, S)
# compute the objective function of the minimum distance estimator based on the CCP approach
objective <- compute_CCP_objective_game(theta_1, theta_2, p_joint_est, A, S, delta, lambda)
  ...
  objective_i <-
    foreach (j = 1:length(theta_i_list),
             .combine = "rbind") %dopar% {
               ...
               theta_j[i] <- theta_ij
               objective_ij <- 
                 compute_CCP_objective_game(theta_j, theta_2, p_joint_est, A, S, delta, lambda)
               return(objective_ij)
             }
