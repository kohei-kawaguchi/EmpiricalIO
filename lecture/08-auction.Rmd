...
- A remark on the private value assumption:
    ...
    - Suppose that bidder $i$ is uncertain about $V$ and the distribution of $V$ is independent of $x_i$ and $x_{-i}$.
    - Then, $\mathbb{E}\{u_i(V, X_i, X_{-i})|X_i = x_i, X_{-i} = x_{-i}\} = \mathbb{E}\{u_i(V, X_i, X_{-i})|X_i = x_i\} \equiv f(x_i)$ for some monotone increasing function $f$.
...
- The likelihood is:
    ...
    - If $m_t > 1$, then $h_t(w_t) \equiv n_t (n_t - 1) F_X(w_t)^{n_t - 1} [1 - F_X(w_t)] f_X(w_t)$.
...
$$
- For this argument to hold, it has to be that $\eta(b)$ is strictly increasing in $b$. Otherwise, for the same $b$, multiple $x$ can be associated.
