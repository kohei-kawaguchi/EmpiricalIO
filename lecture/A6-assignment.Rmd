...
A random number generation inside `%dorng%` is not reproducible. Therefore, we use package `doRNG` to perform reproducible foreach parallel. 
...
$$
Write a function `compute_order_sequential_entry(payoff_m)` that returns the order of entry of the potential entrants given the payoff. Note that if the less profitable entrant finds it profitable to enter then the more profitable entrant still finds it profitable to enter. Then write a function `compute_sequential_entry(X_m, Z_m, EP_m, NU_m, beta, alpha, delta, rho)` that returns the equilibrium vector of entry at a market. To do so, you must find at which entrant the payoff becomes negative for the first time.

```{r}

compute_order_sequential_entry(payoff_m)
