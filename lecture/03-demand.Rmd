...
- Let $z_i = L_i$, the location of consumer $i$ and $x_j = L_j$, the location of the choice on a street $[0, 1]$ with $L_1 < L_2$.
...
\end{equation}
where $x_j$ is a quality of product $j$ and $z_i$ is the consumer's willingness to pay for the quality with $x_j < w_{j + 1}$.
...
\end{equation}
- If $\zeta_{it}$ takes high value, the consumer attaches higher value to the category.
...
\end{equation}
- Under certain distributional assumption on $\zeta_{ig}$ and $\epsilon_{ij}$, the term $\varepsilon_{ij}$ have a cumulative distribution [I forgot the reference! Let me know if you find it]:
\begin{equation}
