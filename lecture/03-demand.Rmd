...
\begin{equation}
x_j(p, w, y_i, z_i) = 1 \Leftrightarrow j = \text{argmax}_{j = 0, 1, \cdots, j}  v(p, w_j, y_i, z_i).
\end{equation}
...
$$
x_j(p, w_j, y_i, z_i, \epsilon_{i}) \equiv 1\{j = \text{argmax}_{k = 0, \cdots, J} v(p, w_k, y_i, z_i) + \epsilon_{ik}\}
$$
for $k = 0, \cdots, J$.
- The __choice probability__ as observed by econometrician is:
$$
s_j(p, w_j, y_i, z_i) \equiv \int x_j(p, w_j, y_i, z_i, \epsilon_{i}) dG(\epsilon_i).
$$
...
&=\mathbb{P}\{v(p, w_j, y_i, z_i) -  v(p, w_k, y_i, z_i) \ge \epsilon_{ik} - \epsilon_{ij}, \forall k \neq j\}\\
& = \text{...after some algebra: leave as an exercise...}\\
&= \frac{\exp[v(p, w_j, y_i, z_i) ]}{\sum_{k = 0}^J \exp[v(p, w_k, y_i, z_i)] }.
\end{split}
\end{equation}
- For example, if:
\begin{equation}
v(p, w_k, y_i) = \beta_i'w_k + \alpha_i (y_i - p_k),
\end{equation}
...
\begin{equation}
\begin{split}
s_{j}(p, w, y_i) &= \frac{\exp[\beta_i'w_j + \alpha_i (y_i - p_j) ]}{\sum_{k = 0}^J \exp[\beta_i'w_k + \alpha_i (y_i - p_k) ]}\\
...
$$
s_{j}(p, w, y_i) = \frac{\exp[\beta_i'w_j - \alpha_i p_j]}{1 + \sum_{k = 1}^J \exp[\beta_i'w_k - \alpha_i p_k]}
$$
...
\begin{equation}
\begin{split}
l(v; x, y, z, w) &= \sum_{i = 1}^N \ln \mathbb{P}\{x_i = x(p, w_j, y_i, z_i)|p, w_j, y_i, z_i\}\\
