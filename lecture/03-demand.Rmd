...
- __Revealed preference theory__: 
    ...
    - If the assumtion is true, we can recover part of the preference from the demand function.
...
- Consider an expenditure function that satisfies the following useful conditions:
    - It allows aggregation (this motivation is less imporatn in recent days).
...
- With the specification of @RichardStone1954, it becomes $\ln P = \sum_{j} w_j \ln p_j$. 
...
- The nonparametric analysis based on the reveald preference is @Blow2008.
...
\begin{equation}
\begin{split}
v_j(p_j, y_i, z_i) &=u(y_i - p_j, 0, \cdots, \underbrace{1}_{x_j}, \cdots, 0, z_i)\\
&= u^*(y_i - p_j, w_j, z_i)\\
&\equiv v(p_j, w_j, y_i, z_i).
\end{split}
\end{equation}
...
- Let $x(p, w, y_i, z_i) = \{x_j(p, w, y_i, z_i)\}_{j = 0, \cdots, J}$ be the demand function of consumer $i$, that is:
\begin{equation}
x_j(p, w, y_i, z_i) = 1 \Leftrightarrow j = \text{argmax}_{j = 0, 1, \cdots, j}  v(p_j, w_j, y_i, z_i).
\end{equation}
...
\begin{equation}
v_{ij} \equiv v(p_j, w_j, y_i, z_i) \equiv s - t |L_i - L_j| - p_j.
\end{equation}
...
\begin{equation}
\begin{split}
&v(p_1, w_1, y_i, z_i) \ge v(p_2, w_2, y_i, z_i)\\
...
\begin{equation}
v_{ij} \equiv v(p_j, w_j, y_i, z_i) = z_i w_j - p_j,
\end{equation}
...
\begin{equation}
\begin{split}
&v(p_{j + 1}, w_{j + 1}, y_i, z_i) \ge v(p_j, w_j, y_i, z_i)\\
...
- $p, w, y_i, z_i$ are __observed__ but $\epsilon_{ij}$ are __unobserved__.
- When the realization of the preference shock is given, the consumer choice is:
$$
x_j(p, w, y_i, z_i, \epsilon_{i}) \equiv 1\{j = \text{argmax}_{k = 0, \cdots, J} v(p_k, w_k, y_i, z_i) + \epsilon_{ik}\}
$$
for $k = 0, \cdots, J$.
- The __choice probability__ as observed by econometrician is:
$$
s_j(p, w, y_i, z_i) \equiv \int x_j(p, w, y_i, z_i, \epsilon_{i}) dG(\epsilon_i).
$$
...
\begin{equation}
\begin{split}
s_{j}(p, w, y_i, z_i) & \equiv \mathbb{P}\{j = \text{argmax}_{k = 0, 1, \cdots, J} v(p, w_k, y_i, z_i) + \epsilon_{ik}  \}\\
&=\mathbb{P}\{v(p, w_j, y_i, z_i) -  v(p_k, w_k, y_i, z_i) \ge \epsilon_{ik} - \epsilon_{ij}, \forall k \neq j\}\\
& = \text{...after some algebra: leave as an exercise...}\\
&= \frac{\exp[v(p_j, w_j, y_i, z_i) ]}{\sum_{k = 0}^J \exp[v(p_k, w_k, y_i, z_i)] }.
\end{split}
\end{equation}
- For example, if:
\begin{equation}
v(p_k, w_k, y_i, z_i) = \beta_i'w_k + \alpha_i (y_i - p_k),
\end{equation}
...
\begin{equation}
\begin{split}
s_{j}(p, w, y_i, z_i) &= \frac{\exp[\beta_i'w_j + \alpha_i (y_i - p_j) ]}{\sum_{k = 0}^J \exp[\beta_i'w_k + \alpha_i (y_i - p_k) ]}\\
&= \frac{\exp[\beta_i'w_j - \alpha_i p_j]}{\sum_{k = 0}^J \exp[\beta_i'w_k - \alpha_i p_k]}
\end{split}
\end{equation}
- If we normalize the characteristics vector so that $w_0 = 0$ holds for the outside option, it becomes:
$$
s_{j}(p, w, y_i, z_i) = \frac{\exp[\beta_i'w_j - \alpha_i p_j]}{1 + \sum_{k = 1}^J \exp[\beta_i'w_k - \alpha_i p_k]}
$$
...
\begin{equation}
\begin{split}
&\mathbb{E}\{\max_{j = 0, \cdots, J} v(p_j, w_j, y_i, z_i) + \epsilon_{ij}\} \\
&= \text{   ...after some algebra: leave as an exercise...}\\
&= \ln \Bigg\{\sum_{j = 0}^J \exp[v(p_j, w_j, y_i, z_i)] \Bigg\} + constant.
\end{split} 
\end{equation} 
...
- Suppose we observe a sequence of income $y_i$, consumer characteristics $z_i$, choice $x_{i}$, product characteristics $w_j$ and price $p_j$.
...
- Then the log likelihood of $\{x_i\}_{i = 1}^N$ conditional on $\{y_i, z_i\}_{i = 1}^N$ and $\{w_j,p_j\}_{j = 1}^J$ is:
\begin{equation}
\begin{split}
l(v; x, y, z, w) &= \sum_{i = 1}^N \ln \mathbb{P}\{x_i = x(p, w, y_i, z_i)|p, w, y_i, z_i\}\\
& = \sum_{i = 1}^N \log \Bigg\{ \prod_{j = 0}^{J} s_{j}(p, w, y_i, z_i)^{x_{ij}} \Bigg\}\\
&= \sum_{i = 1}^N \sum_{j = 0}^J \log s_{j}(p, w, y_i, z_i)^{x_{ij}}.
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\ln s_{j}(p, w, y, z)  &= \ln \Bigg\{ \frac{\exp[v(p_j, w_j, y, z) ]}{\sum_{k = 0}^J \exp[v(p_k, w_k, y, z)] }  \Bigg\}\\
&= v(p_j, w_j, y, z) - \ln\Bigg\{ \sum_{k = 0}^J \exp[v(p_k, w_k, y, z)]  \Bigg\}.
\end{split}
\end{equation}

- Moreover, because of the location normalization of the utility function,
\begin{equation}
s_{0}(p, w, y, z) = \frac{1}{\sum_{k = 0}^J \exp[v(p_j, w_k, y, z)] }.
\end{equation}
- Hence,
\begin{equation}
\ln s_{j}(p, w, y, z) - \ln s_{0}(p, w, y, z) = v(p, w_j, y, z).
\end{equation}
...
\begin{equation}
\min \sum_{(y, z)} \sum_{j = 1}^J \{\ln[s_{j}(y, z)/s_{0}(y, z)] - v(p_j, w_j, y, z)\}^2.
\end{equation} 
- If $v$ is linear in parameter, it is the ordinal least squares:
\begin{equation}
v(p_j, w_j, y_m) = \beta_i' w_j - \alpha_i p_j.
\end{equation}

\begin{equation}
\ln[s_{j}(y, z)/s_{0}(y, z)] = \beta_i' w_j - \alpha_i p_j.
\end{equation}
...
- Notice that:
\begin{equation}
\frac{s_j(p, w, y, z)}{s_{k}(p, w, y, z)} = \frac{\exp[v(p_j, w_j, y, z)]}{\exp[v(p_k, w_k, y, z)]}.
\end{equation}
...
- Let $v(p_j, w_j, y_i, z_i) = \beta_i'w_j - \alpha_i p_j$. Then, we have:
\begin{equation}
e_{jk} =
\begin{cases}
-\alpha p_{j} (1 - s(p_j, w_j, y_i, z_i)) &\text{   if   } k = j\\
\alpha p_{k} s(p_k, w_k, y_i, z_i) &\text{   if   } k \neq j.
\end{cases}
\end{equation}
...
- But the pure characteristics model is computationally not straightfoward.
