...
- This chapter is mostly based on @berry_identification_2006.

### Entry Cost, Mode of Competition, and Market Structure

- Fixed and sunk entry costs and mode of competition are key determinants for market structure [@sutton_chapter_2007]. 
...
2. $\Psi$ is known: Normalize the distribution of $F$.
    ...
    v(x) = \Psi^{-1}[p(x)].
    $$
3. Impose shape restrictions on $v$ [@matzkin_nonparametric_1992]:
...
- @bresnahan_entry_1991 pioneered the analysis of oligopoly entry models.
...
- @tamer_incomplete_2003 considers the identification when there are only two potential entrants and the pure-strategy Nash equilibrium is assumed.
...
- @berry_estimation_1992 considers several extensions to the homogeneous oligopoly models.
...
- @berry_estimation_1992 develops a more general model built on the ideas above.
...
- Based on @berry_estimation_1992.
...
- First, even in @berry_estimation_1992's framework, the identity of entrants were not uniquely predicted.
...
- Return to @tamer_incomplete_2003's bivariate game with heterogeneous profits.
...
- Inference based on the moment inequalities are found in @andrews_inference_2010.
- @ciliberto_market_2009 study the entry exit of airlines when there are heterogeneous competitive effects using the above approach.
...
$$
\widehat{G}_{im}(x_{im}, x_{jm}) \approx G_{im}^{jm}[t_{im}(x_{im}, x_{jm})],
$$
$$
