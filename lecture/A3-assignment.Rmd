...
The deadline is the __March 11 1:30pm__. 
...
Because this takes time, you may want to parallelize the commutation using `%dopar` functionality of `foreach` loop. To do so, first install `doParallel` package and then load it and register the workers as follows:
...
In windows, you may have to explicitly pass packages, functions, and data to the worker by using `.export` and `.package` options as follows:
```{r, echo = TRUE}
...
If you have called a function in a package in this way `dplyr::mutate`, then you will not have to pass `dplyr` by `.packages` option. This is one of the reasons why I prefer to explicitly call the pacakge every time I call a function. If you have compiled your functions in a package, you will just have to pass the package as follows:
```{r, echo = TRUE}
