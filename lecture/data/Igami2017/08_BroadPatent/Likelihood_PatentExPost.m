function f = Likelihood_PatentExPost(Theta,output_type)
% This file is to solve the "ex-post broad patent" counterfactual
% This version: 11/15/2013
...
% Need to solve a different, ex-post game from 1988 to 1998 (only),
% because firms make decisions in 1981--1987, NOT anticipating the "1988 court ruling",
% that is, as if they were playing the baseline game.
for t = (T-1):-1:8  
%tic;
...
%            for nn = 0:14       % Circle around Today's # of New firms
        nb = 1;                 % Only Rodime can produce new HDDs
        nn = 0;                 % Only Rodime can produce new HDDs
        Npe = 0;                % No more entry allowed
        Npe_prime = 0;          % No more entry allowed
                ...
                if no > 0
                    ...
                    z6old = fun6patent(z1, z2, beta, phi, kappa_inc, delta, t); % No innovation allowed
                    z7old = 0;                                                  % No innovation allowed
                    else z6old = 0; z7old = 0; end
                if nb > 0
                    %z8old = fun8(z3, beta, phi);
                    z8old = 0;  % Rodime never exits (or someone else will take over its monopoly/patent anyway)
                    else z8old = 0; end
                if nn > 0
                ...
                while (gap > .01 && iterNE < 10)   
                    ...
                    if no > 0
                        ...
                        z6 = (fun6patent(z1, z2, beta, phi, kappa_inc, delta, t) + z6old)/2;
                        z7 = 0; % No innovation allowed
                        else z1 = 0; z2 = 0; z6 = 0; z7 = 0; end
                    if nb > 0
                        z3 = fun3(z6, z7, z8old, z9old, z10old,...
                            no, nb, nn, Npe, Npe_prime, Vprime);
                        %z8 = (fun8(z3, beta, phi) + z8old)/2;
                        z8 = 0; % Rodime never exits (or someone else will take over its monopoly/patent anyway)
                        else z3 = 0; z8 = 0; end
                    if nn > 0
...
if no > 0
    %V(t,1,no+1,nb+1,nn+1) = Pi(t,1,no+1,nb+1,nn+1) + 0.57722 + fun11(z1, z2, beta, phi, kappa_inc, delta, t);
    V(t,1,no+1,nb+1,nn+1) = Pi(t,1,no+1,nb+1,nn+1) + 0.57722 + fun11patent(z1, z2, beta, phi, kappa_inc, delta, t);
    %log(exp(0) + exp(phi + beta * EV(t+1,1,no+1,nb+1,nn+1));   % No innovation allowed
else V(t,1,no+1,nb+1,nn+1) = 0; end

if nb > 0
    V(t,2,no+1,nb+1,nn+1) = Pi(t,2,no+1,nb+1,nn+1) + 0.57722 + fun12(z3, beta, phi);
    %log(exp(0) + exp(phi + beta * EV(t+1,3,no+1,nb+1,nn+1)));
else V(t,2,no+1,nb+1,nn+1) = 0; end

if nn > 0
    V(t,3,no+1,nb+1,nn+1) = Pi(t,3,no+1,nb+1,nn+1) + 0.57722 + fun13(z4, beta, phi);
    %log(exp(0) + exp(phi + beta * EV(t+1,4,no+1,nb+1,nn+1)));
else V(t,3,no+1,nb+1,nn+1) = 0; end
 
                statenum = statenum + 1;
            end
        %end    % No more nb-loop needed
    %end        % No more nn-loop needed
end
