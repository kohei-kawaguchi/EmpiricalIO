% Simulate counter-factual: If No Preemptive Motives
delete 'Diary_cf_NoPrempt'; diary('Diary_cf_NoPreempt');
...
fprintf('\n  -----------------------------------------');
fprintf('\n |            STRATEGIC  MODEL             |');
fprintf('\n |  WITHOUT INCUMBENTS PREEMPTIVE MOTIVES  |');
fprintf('\n  -----------------------------------------');
...
% In no-preemption CF, entrants ignore incumbents' innovation, 
% and this ignorance should be reflected in (perceived) terminal values, too
for no = 0:11
    for nb = 0:11
        for nn = 0:14
            % Entrants' perceived states: No* = No + Nb, and Nb* = 0 always
            no2 = (no + nb)*(no + nb <= 11) + 11*(no + nb > 11);
            nb2 = 0;
            V(T,3,no+1,nb+1,nn+1) = V(T,3,no2+1,nb2+1,nn+1);
        end
    end
end
...
LL = feval(@Likelihood_NoPreempt, x0, 2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Save & Finish
save Result_cf_NoPreempt.mat ... 
    Pi V EV Policy... % Output of Counterfactual Model Solution
    ...
    SWpv_1981thru98 SWpv_from1999 SWpv SWpv_sum SWpv_1981thru98_ByComponent;
diary off;
warning on;