% Counterfactual of "ex-post broad patent" court ruling in 1988,
% with "License fee" enforcement (rather than preliminary injunction to shut down rivals).
% From 1988, Rodime becomes a non-practicing entity that lives off license fees,
% and everyone's profit from new HDDs will take a "haircut" due to Rodime's patents.

delete 'Diary_cf_ExPostLicenseFee'; diary('Diary_cf_ExPostLicenseFee');
global beta delta State Exit Adopt T Pi V EV Policy;
load b80n4d110_MLEresults.mat;  % Load the baseline solution, to simulate 1981--1987 decisions
load CheckPi.mat;
Policy_1981_1987 = Policy;      % Rename the baseline policy (for 1981--1987 simulations)

% Fraction of new-HDD profits that goes to patent holder (Rodime)
feerate = .75;      
feepct = feerate*100;   % Expressed in percent

% Determine the fraction of the average "profit from new HDDs" for Both-type firms
rationew = zeros(18,1);  % Ratio of profits: (Both's / Old-only's)
for t = 1:18
    rationew(t,1) = sum(sum(sum(Pi(t,2,:,:,:)))) / sum(sum(sum(Pi(t,1,:,:,:))));
end
fracnew = ones(18,1) - (1 ./ rationew);

% "Profits from new HDDs" take a haircut from 1988
Pi_original = Pi;   % Keep the original Pi-matrix for Welfare (PS) calculations
Pi_new = Pi;
for t = 8:18
    Pi_new(t,2,:,:,:) = (1 - feerate) .* fracnew(t,1) .* Pi(t,2,:,:,:) + (1 - fracnew(t,1)) .* Pi(t,2,:,:,:);
    Pi_new(t,3,:,:,:) = (1 - feerate) .* Pi(t,3,:,:,:);
end
Pi = Pi_new;        % Don't need the original Pi-matrix when solving the ex-post (1988-98) game
...
fprintf('\n  --------------------------------------------');
fprintf('\n |    COUNTERFACTUAL: EX-POST BROAD PATENT    |');
fprintf('\n |  implemented by licnese fee of%3.0f percent  |',feepct);
fprintf('\n  --------------------------------------------');
...
Pi_original = Pi_original / scaling;
...
fprintf('\n Solving the ex-post game from 1988 to 1998...')
x0 = [phi, kappa_inc, kappa_ent];
ll = feval(@Likelihood_ExPostLicenseFee,x0,1);   % Solve the game once
...
State_simu(:,4,ns) = npe*ones(18,1);% N_pe is fixed by assumption    
    ...
    for t = 1:17                % Loop 2: for each year
        ...
        if t < 8    % Estimated optimal choice probabilities for 1981--1987
            PrXo = Policy_1981_1987(t,1,No+1,Nb+1,Nn+1);  
            PrEo = Policy_1981_1987(t,2,No+1,Nb+1,Nn+1);
            PrXb = Policy_1981_1987(t,3,No+1,Nb+1,Nn+1);
            PrXn = Policy_1981_1987(t,4,No+1,Nb+1,Nn+1);
            PrEpe = Policy_1981_1987(t,5,No+1,Nb+1,Nn+1);
        else        % Counterfactual choice probabilities for 1988--1997
...
PS_rodime = zeros(18,12,12,15); % Count Rodime's license fee revenue
    ...
    for n1 = 1:12
        for n2 = 1:12
            for n3 = 1:15
                ...
                % Rodime's Profits from license fees
                pi_old  = Pi_original(t,1,n1,n2,n3) - Pi(t,1,n1,n2,n3);
                pi_both = Pi_original(t,2,n1,n2,n3) - Pi(t,2,n1,n2,n3);
                pi_new  = Pi_original(t,3,n1,n2,n3) - Pi(t,3,n1,n2,n3);
                PS_rodime(t,n1,n2,n3) = no * pi_old + nb * pi_both + nn * pi_new;
...
for t = 1:18
    ...
    PS_rodime_simu(t) = PS_rodime(t,No+1,Nb+1,Nn+1);
...
% Social Welfare
SW = CS_simu + PS_simu + PS_rodime_simu + FC_sum - SC_sum;
...
PS_rodime_simu_pv = Beta' * PS_rodime_simu;
FC_simu_pv = Beta' * FC_sum;      % PV of Fixed Cost of Operation
SC_simu_pv = Beta' * SC_sum;      % PV of Sunk Cost of Innovation
SWpv_1981thru98_ByComponent = [CS_simu_pv, PS_simu_pv, PS_rodime_simu_pv, FC_simu_pv, SC_simu_pv];
...
PS_rodime_simu2 = [PS_rodime_simu; PS_rodime_simu(18)/(1-beta)];
FC_simu2 = [FC_sum; FC_sum(18)/(1-beta)];
SC_simu2 = [SC_sum; SC_sum(18)/(1-beta)];
SW2 = [CS_simu2, PS_simu2, PS_rodime_simu2, FC_simu2, -SC_simu2];
SWpv2 = Beta2' * SW2;
...
clear t n1 n2 n3 nb nn no price_old price_new delta_old delta_new...
    ...
    No Nb Nn;
save Result_cf_ExPostLicenseFee.mat... 
    Pi Pi_original V EV Policy Policy_1981_1987... % Output of Counterfactual Model Solution
    State_simu State_avg State_avg_rd... % Output of Task (2)
    Adopt_avg Adopt_avg_rd Adopt_avg_cumul Adopt_avg_cumul_gap...
    CS PS PS_rodime CS_simu PS_simu PS_rodime_simu FC_simu SC_simu SW ... % Output of Task (3)
    SWpv_1981thru98 SWpv_from1999 SWpv_1981thru98_ByComponent ...
    CS_simu2 PS_simu2 PS_rodime_simu2 FC_simu2 SC_simu2 SW2 SWpv2;
diary off;
