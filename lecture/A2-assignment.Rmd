...
9. Repeat this procedure for $t = 1, \cdots 10$ by updating the capital and anticipated shocks, and name the resulting data frame `df_all`.
```{r, eval  = FALSE}
df_all <- df
for (t in 2:T) {
  # change time index
...
result_1st_bw <-
  np::npplregbw(
    data = df_all,
    formula = y_error ~ l_error + k + inv| k + inv
    )
...
objective_olleypakes_2nd(theta, df_all, df_all_1st, W)
```
